  ...
  if (phone) {
    ...
    handlers._tokens.verifyToken(token, phone, (tokenIsValid) => {
      if (tokenIsValid) {
        ...
        callback(401, { Error: "Auth denied" });
      }
    });
    // Lookup the user
  } else {
...
handlers._users.put = (data, callback) => {
  ...
  if (phone) {
    if (firstName || lastName || password) {
      ...
      callback(400, { Error: "Error missing filds to update" });
    }
  } else {
    callback(400, { Error: "Missing required fild" });
  }
};
...
//constainer for all the checks method
handlers._checks = {};

//checks-post
//required: protocol, url, method, sucessCodes, timeoutSecondes
//optional data: none
